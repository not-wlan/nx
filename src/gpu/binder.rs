...
pub fn convert_error_code(err: ErrorCode) -> Result<()> {
    match err {
        ...
        ErrorCode::PermissionDenied => {
            Err(results::lib::gpu::ResultBinderErrorCodePermissionDenied::make())
        }
        ErrorCode::NameNotFound => {
            Err(results::lib::gpu::ResultBinderErrorCodeNameNotFound::make())
        }
        ...
        ErrorCode::AlreadyExists => {
            Err(results::lib::gpu::ResultBinderErrorCodeAlreadyExists::make())
        }
        ...
        ErrorCode::InvalidOperation => {
            Err(results::lib::gpu::ResultBinderErrorCodeInvalidOperation::make())
        }
        ErrorCode::NotEnoughData => {
            Err(results::lib::gpu::ResultBinderErrorCodeNotEnoughData::make())
        }
        ErrorCode::UnknownTransaction => {
            Err(results::lib::gpu::ResultBinderErrorCodeUnknownTransaction::make())
        }
        ...
        ErrorCode::FdsNotAllowed => {
            Err(results::lib::gpu::ResultBinderErrorCodeFdsNotAllowed::make())
        }
        ErrorCode::FailedTransaction => {
            Err(results::lib::gpu::ResultBinderErrorCodeFailedTransaction::make())
        }
...
impl Binder {
    ...
    fn transact_parcel_impl(
        ...
        self.hos_binder_driver.get().transact_parcel(
            ...
            transaction_id,
            0,
            sf::Buffer::from_var(&payload),
            sf::Buffer::from_var(&response_payload),
        )?;
