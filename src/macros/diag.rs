...
macro_rules! diag_assert {
    ($mode:expr, $cond:expr) => {
        if !$cond {
            $crate::diag::assert::assert(
                $mode,
                $crate::results::lib::assert::ResultAssertionFailed::make(),
            );
        }
    };
}
...
macro_rules! diag_log_assert {
    ($logger:ty, $assert_mode:expr => $cond:expr) => {{
        ...
        if $cond {
            ...
            let metadata = $crate::diag::log::LogMetadata::new(
                $crate::diag::log::LogSeverity::Info,
                false,
                msg,
                file!(),
                fn_name,
            ...
            let msg = format!(
                "Assertion failed ({}) -> {}",
                stringify!($assert_mode),
                stringify!($cond)
            );

            let metadata = $crate::diag::log::LogMetadata::new(
                $crate::diag::log::LogSeverity::Fatal,
                false,
                msg,
                file!(),
                fn_name,
